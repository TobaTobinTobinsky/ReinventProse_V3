...
* Descripción: Implementación del gestor de base de datos SQLite con protección anti-null.
*/
...
int DBManager::create_book(const std::string& title, const std::string& author, const std::string& synopsis,
    const std::string& prologue, const std::string& back_cover_text, const std::string& cover_image_path) {
    _connect();
    sqlite3_stmt* stmt;
    const char* sql = "INSERT INTO books (title, author, synopsis, prologue, back_cover_text, cover_image_path) VALUES (?, ?, ?, ?, ?, ?);";
    if (sqlite3_prepare_v2(connection, sql, -1, &stmt, nullptr) != SQLITE_OK) throw DatabaseError(sqlite3_errmsg(connection));
    ...
    sqlite3_bind_text(stmt, 6, cover_image_path.c_str(), -1, SQLITE_TRANSIENT);
    ...
    sqlite3_finalize(stmt); _disconnect(); return id;
}

std::optional<DBRow> DBManager::get_book_by_id(int book_id) {
    _connect(); sqlite3_stmt* stmt; std::optional<DBRow> result = std::nullopt;
    ...
    if (sqlite3_prepare_v2(connection, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            for (int i = 0; i < sqlite3_column_count(stmt); i++) {
                ...
                if (type == SQLITE_INTEGER) row[name] = (long long)sqlite3_column_int64(stmt, i);
                else if (type == SQLITE_FLOAT) row[name] = sqlite3_column_double(stmt, i);
                else { const char* text = (const char*)sqlite3_column_text(stmt, i); row[name] = text ? std::string(text) : std::string(""); }
            }
            result = row;
        }
    }
    sqlite3_finalize(stmt); _disconnect();
    if (!result) throw BookNotFoundError("Libro ID " + std::to_string(book_id) + " no encontrado.");
    return result;
}

std::vector<DBRow> DBManager::get_all_books() {
    _connect(); std::vector<DBRow> books; sqlite3_stmt* stmt;
    ...
    sqlite3_finalize(stmt); _disconnect(); return books;
}

bool DBManager::update_book(int book_id, const std::string& title, const std::string& author, const std::string& synopsis,
    const std::string& prologue, const std::string& back_cover_text, const std::string& cover_image_path) {
    _connect(); sqlite3_stmt* stmt;
    const char* sql = "UPDATE books SET title=?, author=?, synopsis=?, prologue=?, back_cover_text=?, cover_image_path=? WHERE id=?;";
    ...
    sqlite3_finalize(stmt); _disconnect(); return ok;
}
...
int DBManager::create_chapter(int book_id, int chapter_number, const std::string& title, const std::string& content, const std::string& abstract_idea) {
    ...
    sqlite3_prepare_v2(connection, sql, -1, &stmt, nullptr);
    sqlite3_bind_int(stmt, 1, book_id); sqlite3_bind_int(stmt, 2, chapter_number);
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) { sqlite3_finalize(stmt); throw ChapterCreationError("Error al crear capítulo."); }
    int id = (int)sqlite3_last_insert_rowid(connection);
    sqlite3_finalize(stmt); _disconnect(); return id;
}

std::vector<DBRow> DBManager::get_chapters_by_book_id(int book_id) {
    _connect(); std::vector<DBRow> chapters; sqlite3_stmt* stmt;
    ...
    sqlite3_finalize(stmt); _disconnect(); return chapters;
}
...
std::optional<DBRow> DBManager::get_chapter_by_id(int chapter_id) {
    ...
    sqlite3_finalize(stmt); _disconnect(); return result;
}
...
int DBManager::add_concrete_idea(int chapter_id, const std::string& idea) {
    ...
    sqlite3_bind_int(stmt, 1, chapter_id); sqlite3_bind_text(stmt, 2, idea.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(stmt); int id = (int)sqlite3_last_insert_rowid(connection);
    sqlite3_finalize(stmt); _disconnect(); return id;
}

std::vector<DBRow> DBManager::get_concrete_ideas_by_chapter_id(int chapter_id) {
    _connect(); std::vector<DBRow> ideas; sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(connection, "SELECT * FROM concrete_ideas WHERE chapter_id = ? ORDER BY id ASC;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, chapter_id);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            DBRow row; row["id"] = (long long)sqlite3_column_int64(stmt, 0);
            const char* text = (const char*)sqlite3_column_text(stmt, 2);
            row["idea"] = text ? std::string(text) : std::string(""); ideas.push_back(row);
        }
    }
    sqlite3_finalize(stmt); _disconnect(); return ideas;
}
