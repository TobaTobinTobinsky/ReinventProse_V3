...
std::vector<DBRow> DBManager::get_all_books() {
    ...
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(connection, "SELECT * FROM books ORDER BY title;", -1, &stmt, nullptr) == SQLITE_OK) {
...
std::vector<DBRow> DBManager::get_concrete_ideas_by_chapter_id(int chapter_id) {
    ...
    sqlite3_stmt* stmt;
    if (sqlite3_prepare_v2(connection, "SELECT * FROM concrete_ideas WHERE chapter_id = ?;", -1, &stmt, nullptr) == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, chapter_id);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            DBRow row;
